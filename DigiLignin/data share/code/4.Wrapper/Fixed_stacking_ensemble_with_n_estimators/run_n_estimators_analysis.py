# -*- coding: utf-8 -*-
"""
Stacked Ensemble Analysis with Different n_estimators Values
...
def plot_metric(ax, x, y_val, yerr_val, y_test, yerr_test, title, ylabel, color_val, color_test):
    ...
    for i, (x_val, y_val_point, y_test_point) in enumerate(zip(x, y_val, y_test)):
        if i == 0 or i == len(x) - 1 or x_val in [10, 50, 100, 500, 1000]:  # Annotate key points
...
def main():
    ...
    print("="*80)
    print("STACKED ENSEMBLE N_ESTIMATORS PROGRESSION ANALYSIS")
    print("="*80)
    ...
    # Define estimator values to test (as requested)
    ...
    for n_estimators in estimator_values:
        result = run_stacking_with_estimators(X, y, best_features, n_estimators, cv_splits)
    ...
    # Save results
    df_results.to_csv('n_estimators_progression_results.csv', index=False)
    print(f"Results saved to: n_estimators_progression_results.csv")
    ...
    # Add main title
    fig.suptitle('Stacked Ensemble Performance vs Number of Estimators\nBest Feature Combination', 
                 fontsize=16, fontweight='bold', y=0.95)

    # Save the figure in multiple formats
    for ext in ['png', 'tiff', 'pdf', 'svg']:
        plt.savefig(f'n_estimators_progression_plot.{ext}', 
    ...
    # Print summary
    print("\n" + "="*80)
    print("N_ESTIMATORS PROGRESSION ANALYSIS SUMMARY")
    print("="*80)
    ...
    print(f"\nPerformance Improvement Analysis:")
    first_row = df_results.iloc[0]  # n_estimators = 1
    improvement_mae = ((first_row['MAE Validation'] - final_row['MAE Validation']) / first_row['MAE Validation']) * 100
    improvement_r2 = ((final_row['R2 Validation'] - first_row['R2 Validation']) / abs(first_row['R2 Validation'])) * 100
    
    print(f"  From 1 to 1000 estimators:")
    ...
    print("  - n_estimators_progression_results.csv")
    print("  - n_estimators_progression_plot.png/tiff/pdf/svg")
    print("="*80)
