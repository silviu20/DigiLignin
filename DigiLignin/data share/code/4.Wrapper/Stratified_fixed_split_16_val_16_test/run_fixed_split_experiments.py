...
def analyze_split_distribution(X, y, train_idx, val_idx, test_idx):
    ...
    stats_dict = {
        ...
        'train_target_mean': y.loc[train_idx].mean().values[0],
        'train_target_std': y.loc[train_idx].std().values[0],
        'train_target_min': y.loc[train_idx].min().values[0],
        'train_target_max': y.loc[train_idx].max().values[0],
        'val_target_mean': y.loc[val_idx].mean().values[0],
        'val_target_std': y.loc[val_idx].std().values[0],
        'val_target_min': y.loc[val_idx].min().values[0],
        'val_target_max': y.loc[val_idx].max().values[0],
        'test_target_mean': y.loc[test_idx].mean().values[0],
        'test_target_std': y.loc[test_idx].std().values[0],
        'test_target_min': y.loc[test_idx].min().values[0],
        'test_target_max': y.loc[test_idx].max().values[0]
    }
